...
import altair as alt # This import is present but not used in the provided snippets. Keeping for consistency with original file.
...
# NLTK is a core dependency for TextBlob.
# It's crucial to download the necessary corpora for TextBlob to function.
# This block attempts to download them if they are not already present.
# It's placed at the very top to ensure corpora are ready before TextBlob is used.
import nltk
try:
    nltk.data.find('corpora/punkt')
except nltk.downloader.DownloadError:
    nltk.download('punkt')
    st.info("Downloaded NLTK 'punkt' tokenizer.") # Optional: show download status

try:
    nltk.data.find('corpora/averaged_perceptron_tagger')
except nltk.downloader.DownloadError:
    nltk.download('averaged_perceptron_tagger')
    st.info("Downloaded NLTK 'averaged_perceptron_tagger'.")

# TextBlob's sentiment analysis might also benefit from 'wordnet' and 'brown'
try:
    nltk.data.find('corpora/wordnet')
except nltk.downloader.DownloadError:
    nltk.download('wordnet')
    st.info("Downloaded NLTK 'wordnet'.")

try:
    nltk.data.find('corpora/brown')
except nltk.downloader.DownloadError:
    nltk.download('brown')
    st.info("Downloaded NLTK 'brown' corpus.")

# --- DIAGNOSTIC: Test TextBlob import and instantiation ---
# This helps confirm TextBlob is usable immediately after imports and NLTK downloads.
try:
    _test_blob_instance = TextBlob("This is a simple test to ensure TextBlob works.")
    # st.success("✅ TextBlob initialized successfully during startup.") # Comment out for clean UI
except Exception as e:
    st.error(f"❌ Critical Error: TextBlob initialization failed at startup: {e}")
    st.warning("This issue must be resolved for sentiment analysis to function.")
    # If TextBlob is truly broken, it might be best to stop the app or disable relevant features
    # st.stop() # Uncomment to stop app execution if TextBlob is unusable


warnings.filterwarnings('ignore')

pytz=pytz.timezone('Asia/Kolkata') # This line is present in original, keeping it.
# Page configuration
st.set_page_config(
...
class StockAnalyzer:
    def __init__(self):
        ...
        # Combine all stocks into a single list and remove duplicates
    ...
    def get_stock_data(self, symbol, period='1y'):
        ...
        try:
            ...
            # Attempt to get intraday data for the most current price/volume
            ...
            # Get historical data for charts/long-term analysis
            hist = stock.history(period=period)
            info = stock.info # Get comprehensive info dictionary
            
            # Update info with latest intraday data if available
            if not latest_data.empty:
                ...
                elif len(hist) > 1 : # Fallback to historical data's previous close
            ...
            # print(f"Error fetching data for {symbol}: {str(e)}") # For debugging
    ...
    def get_advanced_financial_metrics(self, symbol, info):
        ...
        try:
            ...
            metrics = {
                'PE_Ratio': info.get('trailingPE'), # Use None if not found, handle later
                'ROE': info.get('returnOnEquity'),
                'Debt_to_Equity': info.get('debtToEquity'), 
                'Current_Ratio': info.get('currentRatio'),
                'Market_Cap': info.get('marketCap', 0),
                'Dividend_Yield': info.get('dividendYield'),
                'QoQ_Revenue_Growth': None, # Initialize as None
                ...
                'Retail_Holding': 100 - (random.uniform(15,45) + random.uniform(10,35)), # Still simulated
            ...
            metrics['Retail_Holding'] = max(0, 100 - (metrics.get('DII_Holding',0) + metrics.get('FII_Holding',0))) # Ensure sum is not > 100
            ...
            try:
                if not quarterly_financials.empty and 'Total Revenue' in quarterly_financials.index:
                    ...
                    if len(revenues) >= 2:
                        ...
                        if prev_q_rev and prev_q_rev != 0: metrics['QoQ_Revenue_Growth'] = ((current_q_rev - prev_q_rev) / abs(prev_q_rev)) * 100
                    
                    if len(revenues) >= 5: 
                        current_q_rev = revenues.iloc[0]
                        year_ago_q_rev = revenues.iloc[4]
                        if year_ago_q_rev and year_ago_q_rev != 0: metrics['YoY_Revenue_Growth'] = ((current_q_rev - year_ago_q_rev) / abs(year_ago_q_rev)) * 100
                ...
                if not quarterly_financials.empty and 'Net Income' in quarterly_financials.index: # PAT is often 'Net Income'
                    ...
                    if len(pat) >= 2:
                        ...
                        if prev_q_pat and prev_q_pat != 0: metrics['QoQ_PAT_Growth'] = ((current_q_pat - prev_q_pat) / abs(prev_q_pat)) * 100

                    if len(pat) >= 5:
                        current_q_pat = pat.iloc[0]
                        year_ago_q_pat = pat.iloc[4]
                        if year_ago_q_pat and year_ago_q_pat != 0: metrics['YoY_PAT_Growth'] = ((current_q_pat - year_ago_q_pat) / abs(year_ago_q_pat)) * 100
            except Exception as e:
            ...
            # Fallback to all random values if yfinance fails completely for financials
            return { 
                ...
                'Market_Cap': info.get('marketCap', 0), # Keep market cap from info if available
    ...
    def enhanced_scoring_system(self, metrics):
        ...
        # Use .get() with a default that won't break comparisons (e.g., 0 for growth, inf for ratios)
        pe = metrics.get('PE_Ratio')
        if pe is None: pe = 100 # Default to high PE if not available
        if 0 < pe <= 15: score += 12
        ...
        roe = metrics.get('ROE')
        if roe is None: roe = 0 # Default to 0 if not available
        if roe >= 0.2: score += 12
        ...
        if de is None: de = float('inf') # Default to high debt if not available
        if de <= 0.3: score += 8
        elif de <= 0.6: score += 6
        elif de <= 1.0: score += 3
    ...
    def get_top_movers(self, stock_list, limit=5):
        movers_data = []
        # Ensure we don't try to sample more stocks than available
        sample_size = min(len(stock_list), 30) 
        if sample_size == 0: # Handle empty stock_list
...
def get_sentiment_analysis(text):
    ...
    except Exception as e: # Catch any potential errors from TextBlob
        # print(f"Sentiment analysis failed: {e}") # For debugging
        return "Neutral", "🟡" 
...
def main():
    ...
    if page == "Stock Analysis":
        ...
        with col1_select:
            # Using the combined all_stocks list for selection
        ...
        with col2_custom:
            ...
            if custom_stock_input:
                ...
                # This logic is kept from original, but be aware it might misclassify international stocks
        ...
        if st.button("Analyze Stock", type="primary", key="analyze_stock_button"):
            ...
            if hist_data is not None and info:
                ...
                with m_col1:
                    ...
                    st.metric("P/E Ratio", f"{metrics.get('PE_Ratio'):.2f}" if metrics.get('PE_Ratio') is not None else "N/A")
                with m_col2:
                ...
                with m_col4:
                    ...
                    st.metric("Debt/Equity", f"{metrics.get('Debt_to_Equity'):.2f}" if metrics.get('Debt_to_Equity') is not None else "N/A")
                ...
                with adv_m_col1:
                    ...
                    st.markdown(f"QoQ Revenue: { '🟢' if (qrq_rev_g is not None and qrq_rev_g > 0) else '🔴' if (qrq_rev_g is not None and qrq_rev_g < 0) else ''} {qrq_rev_g:.2f}%" if qrq_rev_g is not None else "QoQ Revenue: N/A")
                    st.markdown(f"YoY Revenue: { '🟢' if (yrq_rev_g is not None and yrq_rev_g > 0) else '🔴' if (yrq_rev_g is not None and yrq_rev_g < 0) else ''} {yrq_rev_g:.2f}%" if yrq_rev_g is not None else "YoY Revenue: N/A")
                    st.markdown(f"QoQ PAT: { '🟢' if (qrq_pat_g is not None and qrq_pat_g > 0) else '🔴' if (qrq_pat_g is not None and qrq_pat_g < 0) else ''} {qrq_pat_g:.2f}%" if qrq_pat_g is not None else "QoQ PAT: N/A")
                    st.markdown(f"YoY PAT: { '🟢' if (yrq_pat_g is not None and yrq_pat_g > 0) else '🔴' if (yrq_pat_g is not None and yrq_pat_g < 0) else ''} {yrq_pat_g:.2f}%" if yrq_pat_g is not None else "YoY PAT: N/A")
                with adv_m_col2:
                    ...
                    st.markdown(f"Div. Yield: {metrics.get('Dividend_Yield',0)*100:.2f}%" if metrics.get('Dividend_Yield') is not None else "Div. Yield: N/A")
                    st.markdown(f"Current Ratio: {metrics.get('Current_Ratio'):.2f}" if metrics.get('Current_Ratio') is not None else "Current Ratio: N/A")
                ...
                with adv_m_col4:
                    ...
                    qoq_dii_c = metrics.get('QoQ_DII_Change')
                    qoq_fii_c = metrics.get('QoQ_FII_Change')
                    st.markdown(f"DII Change: {'🟢' if (qoq_dii_c is not None and qoq_dii_c > 0) else '🔴' if (qoq_dii_c is not None and qoq_dii_c < 0) else ''} {qoq_dii_c:.2f}%" if qoq_dii_c is not None else "DII Change: N/A")
                    st.markdown(f"FII Change: {'🟢' if (qoq_fii_c is not None and qoq_fii_c > 0) else '🔴' if (qoq_fii_c is not None and qoq_fii_c < 0) else ''} {qoq_fii_c:.2f}%" if qoq_fii_c is not None else "FII Change: N/A")
        ...
        st.markdown("Filter stocks based on your preferred financial metrics from **all available stocks**. _Data is fetched live and may take a moment._")

        with st.expander("Set Filtering Criteria", expanded=True):
            c1, c2, c3 = st.columns(3)
            with c1:
                pe_min = st.number_input("Min P/E Ratio", value=0.0, min_value=0.0, step=1.0, format="%.1f", key="picker_pe_min")
            ...
            with c3:
                ...
                yoy_pat_growth_min_pct = st.number_input("Min YoY PAT Growth (%)", value=5.0, min_value=-100.0, step=1.0, format="%.1f", key="picker_yoy_pat_min")
        
        # REMOVED: stock_categories_to_scan multiselect
        ...
        if len(stocks_to_scan_list) > max_scan_limit:
            st.warning(f"Scanning is limited to {max_scan_limit} random stocks from the entire list for performance. Adjust filters for more precise results.")
        ...
        if st.button("Find Matching Stocks", type="primary", key="find_stocks_button"):
            ...
            with st.spinner(f"Scanning {len(stocks_to_scan_list)} stocks... This might take a while."):
                ...
                for i, stock_symbol in enumerate(stocks_to_scan_list):
                    try:
                        # Fetching 1 month historical data for picker for quicker response
                        ...
                        if s_info:
                            ...
                            # Use .get() with appropriate defaults for comparison
                            ...
                            yoy_rev_val = s_metrics.get('YoY_Revenue_Growth') if s_metrics.get('YoY_Revenue_Growth') is not None else -float('inf')
                            yoy_pat_val = s_metrics.get('YoY_PAT_Growth') if s_metrics.get('YoY_PAT_Growth') is not None else -float('inf')
                            ...
                            if (pe_min <= pe_val <= pe_max and
                                roe_val >= roe_min_pct / 100 and # Convert % to decimal
                                (mcap_min_cr * 1e7) <= mcap_val <= (mcap_max_cr * 1e7) and # Convert Cr to actual value
